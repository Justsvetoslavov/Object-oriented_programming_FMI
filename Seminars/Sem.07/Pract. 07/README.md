...
* Помислете как да предефинирате следните оператори така, че имплементацията ви максимално да прилича на `string` от стандартната библиотека:
    * `[]`
    * `+`
    * `+=`
    * `==`
...
Погледнете документацията и имлементирайте колкото се може повече методи.
> За reference: https://www.cplusplus.com/reference/string/string/

Минимумът, който трябва да покриете:

`length` - да връща дължината на низа (без да брои `\0`)  
`empty` - да връща дали низът е празен  
`clear` - да изтрие съдържанието на низа  
`c_str` - да връща низа като `const char*`  
`append` - да добавя един символ към края на низа  
`compare` - сравнява два стринга  
